...
특히 개인적인 경험이지만, **직접 배치 관리 도구를 만드는 것에 부정적**입니다.  
  
파이프라인/여러 SaaS들과의 통합/배포도중 스케줄링이 겹쳤을때 처리방안 등 직접 구현으로 한계가 명확하다고 생각하기 때문인데요.  
...
> 다만 현재는 Spring Cloud Data Flow가 국내에 활성화가 안된 상태라 자료나 커뮤니티가 거의 없다고 보시면 됩니다.
...
(참고: [readlink로 무중단 배치 배포 구성하기](https://jojoldu.tistory.com/315))  
...
관리도구로서 대표 기능이라고 볼 수 있는 대시보드 / 이력관리 / 로그인 / 계정 권한 관리 등등이 **기본으로 지원**됩니다.  
...
특히 계정의 경우 기본적인 ID/Password를 비롯하여 LDAP, Github 등 여러 인증 방식을 지원합니다.
...
위의 기능이 관리자 페이지의 기본 기능이긴하나, **이걸 어찌됐든 개발하는데는 공수가 들어갑니다**.  
  
서버에서 ```yum install``` 명령어만 실행하면 이미 전세계의 개발자들이 사용하는 서비스를 사용할 수 있는데 굳이 **바퀴를 재발명할 필요가 있을까요?**
...
* 스케줄링으로 실행되던 배치를 수동으로 한번 실행해야 할때
  * 매번 스케줄링 시간을 현재시간으로 변경하고, 롤백하고 하다보면 사고가 날 수도 있겠죠?
...
정말 별별 플러그인들이 다 있으며, **직접 플러그인을 만들어** 오픈소스로 제공하기도 합니다.
...
> 비슷한 사례로 **사내 프레임워크/플랫폼을 사용할때 해당 서비스의 담당자가 1~2명**이라면 해당 담당자 퇴사하면 어떻게 관리하실건가요?

### 2-5. 파이프라인

Jenkins와 같은 CI도구의 최대 장점이라고 볼 수 있는 파이프라인도 배치 관리 도구로서 가지는 큰 장점이라고 봅니다.
...
파이프라인이 결국 Job을 묶어주는 것이 가능하기 때문에 **Step이 아닌 Job을 최소 단위로 사용**할 수 있습니다.

* Job과 Step의 1:1 관계로
* 쪼개진 Job들은 Jenkins의 파이프라인이 각각의 Job을 묶어주는 구조로
...
하는 등등 **각각의 Job들을 어떻게 실행할지** 파이프라인이 결정할 수 있습니다.  
...
기본적으로는 지정된 날짜의 데이터를 가공하는 배치였는데, 마이그레이션을 위해 한달치를 가공해야한다면 어떻게 해야할까요?

* 쿼리를 수정해서 Group by코드로 변경하고
* Job Parameter를 시작일/종료일을 받고
* Where 는 ```eq```에서 ```between```으로 변경해야겠죠? 
...
완성된 배치 코드를 어떻게 조립하고 사용할지 완전히 파이프라인에게 맡기면되니 **개발자들은 단일책임원칙을 Batch Job에도 적용**할 수 있게 됩니다.

#### 다양한 파이프라인 관리 방법

이런 파이프라인을 관리할 수 있는 방법들도 다양하게 지원합니다.  
...
* 빈약한 Web Editor 지원
  * 대부분의 CI 도구들에서는 Web Editor (Shell명령어나 파이프라인 스크립트 작성 에디터)에서 **자동 완성, 문법 지원**등이 강력하게 지원됩니다.
    * 예로, Teamcity의 경우 WebEditor에서 현재 사용할 수 있는 환경변수/Teamcity에 등록한 전역변수 등을 자동완성으로 계속 추천해줍니다.
  * Jenkins의 경우 **문법 체크/템플릿 외에는 지원하지 않습니다**.
  * 그러다보니 단순한 코드를 작성할때도 공식 문서를 검색하는 일이 빈번합니다.  
...
* 신뢰할 수 없는 플러그인
  * 예를 들어 Docker 관련 플러그인을 검색해보시면 **20개가 넘는 결과**가 조회 됩니다. 
    * 이 중 어떤걸 써야할까요?
...
* 플러그인 만들기가 쉬운편이 아닙니다.
  ...
  * 다른 CI 도구들이 플러그인 등록 과정을 정말 쉽게 구현한 것과 비교하면 차이가 많이 납니다.
    * 그래도 플러그인 등록하는 분들이 계신걸 보면 대단하신것 같습니다.
...
* 파일 기반으로 구성되다보니 ```rsync``` 등으로 백업서버로 게속 동기화 하지 않는 이상은 **주기적으로 외부파일서버/S3/Github등으로 디렉토리 통째로 업로드** 하는 방법으로만 백업 해야만 합니다.
...
즉, HA가 지원되는 DB가 이미 구성되어있다면 별도로 백업 환경 구성 없이 장애 날 경우에 해당 CI를 설치만하고 DB 주소 연결만 하면 즉시 복구됩니다.
...
* 단점은
  * Jenkins UI와 사용성에 익숙하신 분들에겐 굉장히 어색합니다.
    * Teamcity와 Jenkins 둘다 처음 접하는 분들은 Teamcity의 UI/UX가 훨씬 더 편하고 친숙하다는 의견도 많습니다.
  * 일정 규모이상에서는 유료 플랜이 필요합니다.
  * 결국은 CI/CD 도구이다보니 배치쪽으로 발전 방향을 향하고 있진 않습니다.
